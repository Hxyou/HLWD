...
def train():
    ...
    dataset_test = provider.view_data(cfg, state='test',
                                         batch_size=cfg.batch_size, shuffle=True,
                                         img_sz=cfg.img_size)

    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            images_pl = tf.placeholder(tf.float32, shape=(BATCH_SIZE, NUM_VIEWS, IMG_SIZE, IMG_SIZE, 3))
            ...
            pred_pc = MODEL.MVCNN.inference_multiview_all(images_pl, is_training_pl)
        ...
        saver.restore(sess, save_path=cfg.ckpt_view_model)
...
def test(sess, ops, dataset_test):
    ...
    for batch_idx in range(num_batches):
        imgs, lbls = dataset_test.get_batch(idx=batch_idx, view_num=NUM_VIEWS)
