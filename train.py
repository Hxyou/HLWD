...
def train():
    ...
    with tf.Graph().as_default():
        with tf.device('/gpu:'+str(GPU_INDEX)):
            ...
            pred_pc = MODEL.multi_modal(pointclouds_pl, images_pl, is_training_pl,
                                                                           bn_decay=bn_decay)
            loss_pc = MODEL.get_loss_pc(pred_pc, labels_pl)
            loss = 1 * loss_pc
            ...
            learning_rate_all = get_learning_rate_all(batch_all)
            optimizer_all = tf.train.AdamOptimizer(learning_rate=learning_rate_all)
            train_op_all = optimizer_all.minimize(loss, global_step=batch_all)
        ...
        for epoch in range(MAX_EPOCH):
            ...
            train_one_epoch(sess, ops, train_writer, dataset_train, epoch)
            eval_acc_pc= eval_one_epoch(sess, ops, test_writer, dataset_test)
...
def train_one_epoch(sess, ops, train_writer, dataset_train, epoch):
    """ ops: dict mapping from string to tf ops """
    is_training = True

    num_data, num_batches = dataset_train.get_len()
    print ('num of train data: %d' % num_data)
    print ('num of train batch: %d' % num_batches)
    ...
    if (epoch + 1) % 3 == 0:
        log_string('----' + str(epoch) + ': update all-----')
    else:
        log_string('----' + str(epoch) + ': update part-----')
    ...
    for batch_idx in range(num_batches):
        ...
        # summary, step, _, loss_pc, pred_pc, loss_mv, pred_mv = sess.run([ops['merged'], ops['step'],
        #     ops['train_op'], ops['loss_pc'], ops['pred_pc'], ops['loss_mv'], ops['pred_mv']], feed_dict=feed_dict)

        if (epoch+1) % 3 == 0:
            summary, step, _, loss_pc, pred_pc= sess.run([ops['merged'], ops['step'],
                ops['train_op_all'], ops['loss_pc'], ops['pred_pc']], feed_dict=feed_dict)
        else:
            summary, step, _, loss_pc, pred_pc= sess.run([ops['merged'], ops['step'],
                ops['train_op'], ops['loss_pc'], ops['pred_pc']], feed_dict=feed_dict)
        ...
        loss_sum_pc += (loss_pc*BATCH_SIZE)

        # pred_mv = np.argmax(pred_mv, 1)
        # correct_mv = np.sum(pred_mv == lbls)
        # total_correct_mv += correct_mv
        # loss_sum_mv += (loss_mv*BATCH_SIZE)
...
def eval_one_epoch(sess, ops, test_writer, dataset_test):
    ...
    for batch_idx in range(num_batches):
        ...
        feed_dict = {ops['pointclouds_pl']: pcs,
                     ...
                     ops['labels_pl']: lbls,
                     ops['is_training_pl']: is_training}
        summary, step, loss_val_pc, pred_val_pc = sess.run([ops['merged'], ops['step'],
            ops['loss_pc'], ops['pred_pc']], feed_dict=feed_dict)
